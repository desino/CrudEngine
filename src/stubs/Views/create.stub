...
<div class="row g-0 align-items-center">
    <div class="col-12">
        <form action="{{ route('{{camelCasePluralName}}.create') }}" class="needs-validation" method="POST" novalidate name="{{snakeCaseSingularName}}_create_form" id="{{snakeCaseSingularName}}_create_form">
            @csrf
            <div class="card border-0 pt-3">
                <div class="card-header bg-transparent border-0 row d-flex align-items-center p-0">
                    <div class="col-12 col-md-9 col-lg-10">
                        ...
                        <div class="my-2 small fst-italic clearfix text-app_lblue">
                            {!! trans('messages.create_{{snakeCaseSingularName}}_page_desc') !!}
                        </div>
                    </div>
                ...
                <div class="card-footer bg-transparent p-0 d-grid gap-1">
                    <button class="btn btn-app_dblue fw-bold border-0 text-white" type="submit" style="outline: none;">
                        {{ trans('messages.create_{{snakeCaseSingularName}}_form_submit_btn_text') }}
                    </button>
                </div>
            </div>
        </form>
